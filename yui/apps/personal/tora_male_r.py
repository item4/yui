...
async def watch(bot, sess):
    url = (
        'https://ec.toranoana.jp/tora_r/ec/cot/genre/GNRN00001186/'
        'all/all/?indexSummaryResultType=1&indexLocalCode=0030'
    )
    headers = {
        'Cookie': 'adflg=0',
    }
    ...
    cache = get_or_create_cache('personal-tora-male-r', sess)
    ...
    if attachments:
        ...
        await retry(bot.api.chat.postMessage(
            channel=C.sao.get(),
            as_user=True,
            text='토라노아나(남성/성인)에 소드 아트 온라인 신간이 올라왔어요!',
